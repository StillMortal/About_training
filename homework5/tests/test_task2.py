...
def test_decorated_func_prints_and_returns_correct_value(capsys):
    decorated_func = print_result(sum_of_only_three_parameters)
    returned_value = decorated_func(1, 2, 3, 4)
    captured = capsys.readouterr()

    assert captured.out == "6\n" and returned_value == 6


def test_decorated_func_has_its_own_doc_and_name():
    decorated_func = print_result(sum_of_only_three_parameters)
    decorated_func(1, 2, 3, 4)

    assert (
        decorated_func.__name__ == sum_of_only_three_parameters.__name__
        and decorated_func.__doc__ == sum_of_only_three_parameters.__doc__
    )
